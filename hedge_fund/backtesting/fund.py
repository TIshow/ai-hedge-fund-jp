...
def backtest_fund(
    ...
    # are multiplied on the ex-date, before execution and valuation, so marks
    # and positions stay consistent.
    ...
    for i, session in enumerate(dates):
        if share_splits is not None and i > 0:
            for ticker in sorted(broker.positions()):
                for _day, ratio in share_splits(ticker, dates[i - 1], session):
